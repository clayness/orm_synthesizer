...
public class FrontParser {
	...
	public String createMappingRun(String spec) {
		...
		fName = fName + File.separator + mappingrunFileName[0] + "_mapping_run." + mappingrunFileName[1];

		try (BufferedWriter fWriter = Files.newBufferedWriter(Paths.get(fName));
			...
			out.println("}");
			out.println("run show for " + String.valueOf(this.counter + 1));
		} catch (IOException ex) {
			Logger.getLogger(FrontParser.class.getName()).log(Level.SEVERE, null, ex);
		}
		return fName;

...
public class DataProvider {
	...
	public HashMap<String, ArrayList<CodeNamePair<String>>> getTables() {
		return this.tableItems;
	}

	public ArrayList<CodeNamePair<String>> getParents() {
		return this.parents;
	}
	...
	public ArrayList<CodeNamePair<String>> getTypes() {
		...
		for (CodeNamePair<String> pair : this.types) {
			CodeNamePair<String> tmp = new CodeNamePair<String>(pair.getFirst().toString(),
	...
	/**
	 * replace all items with $ symbol and add them to the table, then delete all
	 * table items with $ symbol
	 */
	public void refineTable(ArrayList<Sig> sigs) {
		...
		for (Map.Entry<String, ArrayList<CodeNamePair<String>>> entry : tableItemSet) {
			...
			ArrayList<CodeNamePair<String>> tableContents = entry.getValue();

			if (tableName.contains("$")) {
				tableName = getSecondByFirst(tableName);
				if (!tmpTableItems.containsKey(tableName)) {
					tmpTableItems.put(tableName, new ArrayList<CodeNamePair<String>>());
				}

				int arraySize = tableContents.size();
				for (int i = 0; i < arraySize; i++) {
					String firstField = tableContents.get(i).getFirst().toString();
					if (firstField.contains("$")) {
						firstField = this.getSecondByFirst(firstField);
					}
					String secondField = tableContents.get(i).getSecond().toString();
...
//        }
		...
		for (int i = 0; i < this.pairs.size(); i++) {
			if (this.pairs.get(i).getFirst().toString().startsWith("Table")) {
				...
				for (j = 0; j < this.parents.size(); j++) {
					if (this.pairs.get(i).getSecond().toString()
							.equalsIgnoreCase(this.parents.get(j).getFirst().toString())) {
						break;
					}
				}
		...
		for (int i = 0; i < this.parents.size(); i++) {
			...
			ArrayList<CodeNamePair<String>> tmpList1 = tmpTableItems.get(this.parents.get(i).getSecond().toString());
			if (tmpList1 == null) {
				continue;
			}
			for (int j = 0; j < tmpList1.size(); j++) {
				...
				if (tmp.contains("$")) {
					...
					tmpTableItems.get(this.parents.get(i).getSecond().toString()).get(j).setSecond(tmp1);
					// tmpList1.get(i).setSecond(tmp1);
				}
			}

			ArrayList<CodeNamePair<String>> tmpList = tmpTableItems.get(this.parents.get(i).getFirst());
			for (int j = 0; j < tmpList.size(); j++) {
				if (!hasItemInArray(tmpTableItems.get(this.parents.get(i).getSecond()), tmpList.get(j))) {

					tmpTableItems.get(this.parents.get(i).getSecond()).add(tmpList.get(j));
				}
			}
	...
	public boolean hasItemInArray(ArrayList<CodeNamePair<String>> list, CodeNamePair<String> pair) {
		...
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i).getFirst().toString().equalsIgnoreCase(pair.getFirst().toString())
					&& list.get(i).getSecond().toString().equalsIgnoreCase(pair.getSecond().toString())) {
				return true;
			}
		}
		return has;
	...
	public CodeNamePair<String> hasSrcDst(String tableName) {
		...
		for (CodeNamePair<String> pair : items) {
			...
			if (srcdst.getFirst().length() > 0 && srcdst.getSecond().length() > 0) {
				return srcdst;
			}
		}
		return srcdst;
	...
	public CodeNamePair<String> getMultiplicity(String tableName) {
		...
		for (CodeNamePair<String> pair : items) {
			...
			if (srcdst_mul.getFirst().length() > 0 && srcdst_mul.getSecond().length() > 0) {
				return srcdst_mul;
			}
		}
		return srcdst_mul;
	...
	public boolean isForeignKey(String table, String field) {
		...
		ArrayList<CodeNamePair<String>> table1 = this.tableItems.get(table);

		for (int i = 0; i < table1.size(); i++) {
			if (table1.get(i).getFirst().toString().equalsIgnoreCase("foreignKey")) {
				if (table1.get(i).getSecond().toString().equalsIgnoreCase(field)) {
					return true;
				}
			}
		}
	...
	/**
	 * Find the table name that ID is its primaryKey and ID is not its foreignKeyStr
	 *
	 * @param ID: input id
	...
	public String tableNameByID(String ID) {
		...
		for (Map.Entry<String, ArrayList<CodeNamePair<String>>> entry : tableItemSet) {
			...
			ArrayList<CodeNamePair<String>> tmpArray = entry.getValue();
			int arraySize = tmpArray.size();
			for (int i = 0; i < arraySize; i++) {
//                if (tmpArray.get(i).getFirst().toString().equalsIgnoreCase("attr")) {
				if (tmpArray.get(i).getFirst().toString().equalsIgnoreCase("ID")) {
					if (tmpArray.get(i).getSecond().toString().equalsIgnoreCase(ID)) {
//                        tableName = entry.getKey();
						// System.out.println("Find Table: "+ tableName);
						// check the ID is also in attr
						for (int j = 0; j < arraySize; j++) {
							if (tmpArray.get(j).getFirst().equalsIgnoreCase("attr")) {
								if (tmpArray.get(j).getSecond().equalsIgnoreCase(ID)) {
									return entry.getKey();
								}
							}
						}
	...
	public String hasParent(String child) {
		String parent = null;
		for (int i = 0; i < this.parents.size(); i++) {
			System.out.println(this.pairs.get(i).getFirst().toString() + "   " + child);
	...
	public int hasMultipleItem(String tableName, String item) {
		int key = 0;
		ArrayList<CodeNamePair<String>> table1 = this.tableItems.get(tableName);
...
//            if (table1.get(i).getFirst().toString().equalsIgnoreCase("Id")) {
			if (table1.get(i).getFirst().toString().equalsIgnoreCase("primaryKey")) {
				if (table1.get(i).getSecond().toString().equalsIgnoreCase(field)) {
					isID = true;
					break;
				}
	...
	public boolean isParent(String code) {
		for (int i = 0; i < this.parents.size(); i++) {
			if (this.parents.get(i).getSecond().toString().equalsIgnoreCase(code)) {
				return true;
			}
		}
		return false;
	...
	public boolean hasPairCode(String code) {
		...
		for (i = 0; i < this.pairs.size(); i++) {
			CodeNamePair<String> tmp = this.pairs.get(i);
	...
	public boolean writeIntoFile(String filename) throws IOException {
		...
		try (OutputStream oFile = Files.newOutputStream(Paths.get(filename));
				PrintStream pPRINT = new PrintStream(oFile)) {
			...
			for (Map.Entry<String, ArrayList<CodeNamePair<String>>> entry : entrySet) {
				...
				// primaryKeyStr will write to file at the end of every create table block
				...
				for (int i = 0; i < arraySize; i++) {
					// the itemName is the name of this item, like customerID, orderID
					String itemName = tableItems.get(i).getSecond().toString();
					// if(itemName.contains("$")){
					// itemName = itemName.split("$")[0];
					// }
					// itemType is the type of the item, for example, the type of customerID is
					// Ineteger
					...
					if (caseName.equalsIgnoreCase("fields")) {
						...
						pPRINT.println("KEY `FK_" + tableName + "_" + itemName + "_idx` (`" + itemName + "`),");

						// add constrains for ths table
						// find the primary tablename
						String pkTable = tableNameByID(itemName);
						if (1 == FKNum) {
							foreignKeyStr = foreignKeyStr + "  ADD CONSTRAINT `FK_" + tableName + "_" + itemName + "` "
									+ "FOREIGN KEY (`" + itemName + "`) REFERENCES `" + pkTable + "` (`" + itemName
									+ "`) " + "ON DELETE CASCADE ON UPDATE CASCADE;\n";
						} else if (FKNum > 1) {
							lastFKCounter++;
							if (lastFKCounter < FKNum) {
								foreignKeyStr = foreignKeyStr + "  ADD CONSTRAINT `FK_" + tableName + "_" + itemName
										+ "` " + "FOREIGN KEY (`" + itemName + "`) REFERENCES `" + pkTable + "` (`"
...
//        tmp_path += "Inserts\\" + moduleName;
		...
		try (OutputStream oFile = Files.newOutputStream(Paths.get(fileName));
				PrintStream pFile = new PrintStream(oFile)) {
			int rows = 100000;
			...
			for (Map.Entry<String, ArrayList<CodeNamePair<String>>> entry : entrySet) {
				String tableName = entry.getKey();
				ArrayList<CodeNamePair<String>> items = entry.getValue();
				...
				while (rows_for_one_table++ < rows) {
					...
					String statement = "INSERT INTO `" + tableName + "` (" + columnNames + ") VALUES (" + values + ");";

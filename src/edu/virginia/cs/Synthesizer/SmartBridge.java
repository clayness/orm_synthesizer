...
//    static String archStyle = "";
    ...
    /*
     * args[0]: solution folder
     * args[1]: alloy OM
     * args[2]: max solution number parameter
     * args[3] (optional): store all solution? default value: false
     */
...
//        String mergedFile = workspace + "\\"+ appType + "\\" + appDesc + "_" + archStyle + ".als";
...
//    public SmartBridge(String solutionDirectory, String filename, String appDescp, String appTypep, String archStylep, int maxSol) throws Err {
...
//                          System.out.println(sensorName);
        ...
        trimmedFilename = solutionDirectory + "/" + appFileName.substring(0, appFileName.length()-12);
...
//                System.out.println("\n-----------------------------------------");
//                System.out.println("# Eq.Classes / # overall solutions : " +solutionsMV.size() +" / " + --solutionNo);
//                System.out.println("Current Time: "+now());
//                file.println("\n-----------------------------------------");
//                file.println("# Eq.Classes / # overall solutions : " +solutionsMV.size() +" / " + solutionNo);
//                file.println("Current Time: "+now());
...
//            }
            ...
            while (!isFinished) {
                if (solutionNo > maxSol) {
                    ...
                    System.out.println("Current Time: "+now());
                    file.println("\n-----------------------------------------");
                    file.println("# Eq.Classes: " +solutionsMV.size() +" / " + solutionNo);
                    file.println("Current Time: "+now());
                    break;
                }
                if (solution.satisfiable()) {
                    ...
                    if(isNewSolution || storeAllSolutions){
                        solution.writeXML(trimmedFilename + "_Sol_" + solutionNo + ".xml");  // This writes out "answer_0.xml", "answer_1.xml"...
    ...
    private boolean measureMetric(Module root, A4Solution solution, int solutionNo) throws Err {
...
//            System.out.println("className: " + className);
        ...
        for (Iterator<String> resultIterator = classNames.iterator(); resultIterator.hasNext();) {
            className = resultIterator.next();
            String queryTATI = "#"+className+".*(~parent).~tAssociate";
            ArrayList queryResults = e.query(queryTATI);
            Integer valueTATI = Integer.parseInt(queryResults.get(0).toString());
...
//            Integer valueANV = Integer.parseInt(queryResults1.get(0).toString()) * Integer.parseInt(queryResults2.get(0).toString());
            Integer valueANV = value1 * value2;
            overallANV += valueANV;
//            System.out.println("ANV(" + className+")= " +valueANV);
            solutionMV.setANV_detail(solutionMV.getANV_detail()+"\nANV(" + className+")= " +valueANV);
        }
...
//        for (Iterator<String> resultIterator = tables.iterator(); resultIterator.hasNext();) {
...
//
//            overallNIC += valueNIC;
////            solutionMV.setNIC_detail(solutionMV.getNIC_detail() + "\nNIC(" + tableName + ")= " + valueNIC);
//        }
////        file.println("overall_NIC(solution:" + solutionNo+")= " +overallNIC);
...
//        if (eqClassNo==0) {
            ...
            for (Iterator<MetricValue> resultIterator = solutionsMV.iterator(); resultIterator.hasNext();) {
                ...
                if(solutionMV.getTATI()>=instance.getTATI() && solutionMV.getNCT()>=instance.getNCT() &&
...
//                System.out.println("Solution #" + solutionNo + " is a pareto Optimal Solution.");
            ...
            isNewSolution = true;
//            solution.writeXML(trimmedFilename + "_Sol_" + solutionNo + ".xml");  // changed to write all solutions before calling this method

//            System.out.println("-----------------------------------------");
//            file.println("Solution #" + solutionNo + " has been generated.");
//            file.println("Current Time: "+now());
            file.println(solutionMV.getTATI_detail());
            file.println("Overall_TATI(solution:" + solutionNo+")= " +overallTATI);
            file.println(solutionMV.getNCT_detail());
            file.println("Overall_NCT(solution:" + solutionNo+")= " +overallNCT);
            file.println(solutionMV.getNCRF_detail());
            file.println("Overall_NCRF(solution:" + solutionNo+")= " +overallNCRF);
            file.println(solutionMV.getANV_detail());
            file.println("Overall_ANV(solution:" + solutionNo+")= " +overallANV);
//            file.println(solutionMV.getNIC_detail());
            file.println("Overall_NIC(solution:" + solutionNo+")= " +overallNIC);
            file.println("Overall_NFK(solution:" + solutionNo+")= " +valueNFK);
            file.println("Eq.Classes: " +solutionsMV.size() +" / " + solutionNo);
...
//            System.out.println("-----------------------------------------");
    ...
    private void measureNIC(Evaluator e, ArrayList<String> classNames) {
        ...
        for (Iterator<String> resultIterator = classNames.iterator(); resultIterator.hasNext();) {
            ...
            if(queryResults1.size()>0 && !visitedTables.contains(queryResults1.get(0).toString())){
                visitedTables.add(queryResults1.get(0).toString());
                String queryNIC2 = "#("+className+".~tAssociate.fields.fAssociate.~attrSet)";
                ArrayList queryResults2 = e.query(queryNIC2);
                Integer value1 = Integer.parseInt(queryResults2.get(0).toString());
                value1 = value1 < 0? 16+value1:value1;
                overallNIC += value1;
            }
        }
